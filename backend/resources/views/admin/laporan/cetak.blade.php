...
<body class="bg-gray-200 font-sans text-sm text-gray-800">
    <div class="no-print max-w-5xl mx-auto mt-4 flex flex-wrap gap-2 justify-between items-center bg-white p-3 rounded-lg shadow">
        <a href="{{ route('admin.laporan.index', request()->only(['start_date', 'end_date', 'status'])) }}"
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('admin.laporan.cetak', array_merge(request()->only(['start_date', 'end_date', 'status']), ['output' => 'pdf'])) }}"
                target="_blank"
                class="bg-gray-800 hover:bg-gray-900 text-white px-4 py-2 text-xs font-semibold rounded-lg transition">
                Buka PDF
            </a>
            <a href="{{ route('admin.laporan.cetak', array_merge(request()->only(['start_date', 'end_date', 'status']), ['output' => 'pdf', 'download' => '1'])) }}"
